...
class ModelRequestNode(AgentNode[DepsT, NodeRunEndT]):
    ...
    async def _build_retry_node(
        ...
        """
        retry_node = await _create_retry_node(ctx, run_context, error)
...
async def _create_retry_node(
    ...
    Increments the retry counter and creates a new request with a RetryPromptPart.
    Shared by ModelRequestNode and ContinueRequestNode.
    """
    ctx.state.increment_retries(ctx.deps.max_result_retries, error=error)
    m = _messages.RetryPromptPart(content=error.message)
    instructions = await ctx.deps.get_instructions(run_context)
    return ModelRequestNode[DepsT, NodeRunEndT](_messages.ModelRequest(parts=[m], instructions=instructions))
...
class ContinueRequestNode(AgentNode[DepsT, NodeRunEndT]):
    ...
    async def run(
        ...
        new_response, run_context, request_context = await self._request(ctx)
        return await self._finish_handling(ctx, run_context, request_context, new_response)
    ...
    async def _run_stream(
        ...
        await self._finish_handling(ctx, run_context, request_context, new_response)

    async def _request(
        self, ctx: GraphRunContext[GraphAgentState, GraphAgentDeps[DepsT, NodeRunEndT]]
    ) -> tuple[_messages.ModelResponse, RunContext[DepsT], ModelRequestContext]:
        """Make a single non-streaming continuation request.

        Returns the raw model response along with the run and request contexts,
        so the caller can pass them to `_finish_handling`.
        """
        self._check_continuation_limit(ctx)
        run_context, request_context = await self._prepare_continuation(ctx)
        ...
        return response, run_context, request_context
    ...
    async def _finish_handling(
        ...
        """Run after_model_request hook and process the continuation response.

        On success, merges the response into history and sets `_result` to the next node.
        On ModelRetry, commits the partial response and builds a retry node.
        """
        try:
            ...
            # Hook rejected the continuation response — commit partial progress and retry
            self._process_response(ctx, new_response)
            retry_node = await _create_retry_node(ctx, run_context, e)
            self._result = retry_node
            return retry_node
