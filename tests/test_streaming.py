...
async def test_continue_request_node_streams_multiple_continuations() -> None:
    """ContinueRequestNode transitions to a new ContinueRequestNode when streaming returns state='suspended'."""
    ...
    async with agent.iter('test multi-continuation streaming') as run:
        ...
        # First ContinueRequestNode: streams one continuation that returns 'suspended'
        async with node.stream(run.ctx) as stream:
            async for event in stream:
                streamed_events.append(event)

        # Transition produces a new ContinueRequestNode (still suspended)
        next_node = await run.next(node)
        assert Agent.is_continue_request_node(next_node)

        # Second ContinueRequestNode: streams one continuation that returns 'complete'
        async with next_node.stream(run.ctx) as stream:
            async for event in stream:
                streamed_events.append(event)
    ...
    assert request_calls == 1
    # stream_func was called twice: once per ContinueRequestNode
