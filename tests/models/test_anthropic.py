...
async def test_pause_turn_streaming_continuation_wrap_task_error(allow_model_requests: None):
    """Test that an error in wrap_model_request after stream completes is handled via on_model_request_error."""
    from dataclasses import dataclass, field
    ...
    @dataclass
    class ErrorAfterHandlerCapability(AbstractCapability[Any]):
        """Capability that raises after the handler returns, simulating post-handler middleware failure."""
        ...
        async def wrap_model_request(
            ...
            *,
            ...
            self.call_count += 1
            # Only raise on continuation requests (call_count > 1 since first is the initial request)
            if self.call_count > 1:
                raise RuntimeError('capability post-handler error')
            return response

    c1 = completion_message([BetaTextBlock(text='first', type='text')], BetaUsage(input_tokens=10, output_tokens=5))
    c1.stop_reason = 'pause_turn'
    c2 = completion_message([BetaTextBlock(text='recovery', type='text')], BetaUsage(input_tokens=10, output_tokens=5))

    final_stream: list[MockRawMessageStreamEvent] = [
        BetaRawMessageStartEvent(
            type='message_start',
            message=BetaMessage(
                id='msg_final',
        ...
        BetaRawContentBlockStartEvent(
            type='content_block_start',
            index=0,
            content_block=BetaTextBlock(type='text', text='streamed'),
        ),
        BetaRawContentBlockStopEvent(type='content_block_stop', index=0),
        BetaRawMessageDeltaEvent(
            type='message_delta',
            delta=Delta(stop_reason=cast(Any, 'end_turn')),
            usage=BetaMessageDeltaUsage(input_tokens=10, output_tokens=5),
        ),
        BetaRawMessageStopEvent(type='message_stop'),
    ]

    cap = ErrorAfterHandlerCapability()
    mock_client = cast(
        AsyncAnthropic,
        MockAnthropic(messages_=[c1, c2], stream=[[], final_stream]),
    )
    model = AnthropicModel('claude-haiku-4-5', provider=AnthropicProvider(anthropic_client=mock_client))
    agent = Agent(model, capabilities=[cap])

    # The wrap_model_request raises after handler returns during streaming continuation.
    # on_model_request_error should be called, but the default implementation re-raises.
    with pytest.raises(RuntimeError, match='capability post-handler error'):
        async with agent.iter('test prompt') as agent_run:
            node = agent_run.next_node
